...
switch ($acao) {
    ...
    case 'inserirAtendimento';
        ...
        inserirAtendimento();
        break;
    case 'inserirInteracao';
        exibeVar_dump ();
        echo 'Insere a interacao no banco de dados';
        inserirInteracao();
        break;
    default:
...
function paginaConsulta() {
    ...
    <div id="pesquisa" class="row">
        ...
        <form action="?p=page2" role="form" method="post">
            <div class="col-md-6">
...
function viewAtendimentos() {
    ...
    switch ($row) {
        ...
        case '1':
            ...
            <a data-toggle="modal" href="?p=page2#newAtendimento<?php
            echo $idCliente;
                        ...
                        <div class="modal-body">
                            ...
                            switch ($acao) {
                                case 'cadastrarAtendimento':
                                    ...
                                    $nomePessoa = $cliente->nomePessoa;
                                    ?>
                                    <form class="form-wrapper uny ocorrencia" name="frmOcorrencia" enctype="multipart/form-data" action="?p=page2&acao=inserirAtendimento" method="post" role="form" novalidate="novalidate">
                            ...
                            <div class="modal fade" id="Interacao<?php echo $idAtendimento; ?>">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                                            <h4 class="modal-title"><?php echo 'Interacoes para o atendimento ' . $idAtendimento ?></h4>
                                        </div>
                                        <div class="container"></div>
                                        <div class="modal-body">
                                            <?php
                                            echo 'O atendimento registrado por ' . $idCliente . ' sobre ' . $titulo . ', solicitado por ' . $idCliente . ', no dia ' . $dataCriacao . ' tem as seguintes interacoes<br/>';
                                            $interacaoFind = readFind("SELECT  * FROM  interacoes WHERE  idAtendimento='$idAtendimento' ORDER BY  idInteracao");

                                            $row = 0;
                                            foreach ($interacaoFind as $interacao):
                                                //$idInteracao = $interacao->idInteracao;
                                                $idAtendimento = $interacao->idAtendimento;
                                                $newInteracao = $interacao->interacao;
                                                $newdata = $interacao->data;
                                                $usuario = $interacao->usuario;

                                                echo 'No dia tal foi realizado ' . $newInteracao . ' por ' . $usuario . ' no dia ' . $newdata . '<br/>'
                                                . '<br>';
                                            endforeach;
                                            ?>
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    ...
                                    <form data-toggle="modal" name="frmInteracao" action="?p=page2&acao=inserirInteracao" method="post" role="form">                                        
                                        <input id="idCliente" type="hidden" name="idAtendimento" value="<?php echo $idAtendimento; ?>">           
                                        <div class="form-group clearfix">
                                            <div class="col-md-12">
                                                <label class="control-label" for="interacao">Interação*:</label>
                                                <!-- type email used by jquery validate -->
                                                <textarea rows="8" cols="100%" name="interacao" id="interacao" class="form-control" required=""></textarea>
                                            </div>
                                        </div>
                                        <div class="form-group clearfix">
                                            <div class="col-md-8">
                                                <button type="button" class="btn btn-default" data-dismiss="modal">Fechar</button>
                                                <button type="submit" class="btn btn-primary" value="Submit">Salvar Interacao</button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
        ...
        function inserirAtendimento() {
            ...
            $atendimentoAdd = insert('atendimentos', $attributes);
            header('location:../pages/template.php?p=page2&cliente=$cliente&acao=viewAtendimentos');
            exit;
            //echo "<meta HTTP-EQUIV='Refresh' CONTENT='xsegundos; URL=../pages/template.php?p=page2&cliente=$cliente&acao=viewAtendimentos'>";
        }

        function inserirInteracao() {
            ...
            $interacaoAdd = insert('interacoes', $attributes);
            openModal();            
      }
      function openModal(){
          echo 'chamar modal';
